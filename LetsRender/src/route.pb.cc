...
void RoadMessage::SharedCtor() {
  _cached_size_ = 0;
  osm_id_ = 0;
  road_name_ = const_cast< ::std::string*>(&::google::protobuf::internal::kEmptyString);
...
void RoadMessage::SharedDtor() {
  if (road_name_ != &::google::protobuf::internal::kEmptyString) {
    delete road_name_;
  }
...
bool RoadMessage::IsInitialized() const {
  ...
  for (int i = 0; i < geometry_size(); i++) {
    if (!this->geometry(i).IsInitialized()) return false;
  }
  return true;
}
...
bool Route::IsInitialized() const {
  ...
  for (int i = 0; i < path_element_size(); i++) {
    if (!this->path_element(i).IsInitialized()) return false;
  }
  return true;
}
